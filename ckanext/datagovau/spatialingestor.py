...
def _get_blacklisted_orgs():
    return set(
        toolkit.aslist(
            config.get("ckanext.datagovau.spatialingestor.org_blacklist", [])
        )
    )
...
def _get_blacklisted_pkgs():
    return set(
        toolkit.aslist(
            config.get("ckanext.datagovau.spatialingestor.pkg_blacklist", [])
        )
    )
...
def _get_target_formats():
    return set(
        toolkit.aslist(
            config.get("ckanext.datagovau.spatialingestor.target_formats", [])
        )
    )
...
def _get_source_formats():
    return set(
        toolkit.aslist(
            config.get("ckanext.datagovau.spatialingestor.source_formats", [])
        )
    )
    ...
    if dataset and "resources" in dataset:
        for resource in dataset["resources"]:
            ...
            if "/geoserver" in resource["url"]:
                ...
                "shapefile" in _format
                and _valid_source_format("shapefile")
                or ("shz" in _format and _valid_source_format("shz"))
            ):
                shp.append(resource)
            elif ("tab" in _format and _valid_source_format("tab")) or (
                "mapinfo" in _format and _valid_source_format("mapinfo")
            ):
                tab.append(resource)
...
def _set_geoserver_ownership(data_dir):
    uid = pwd.getpwnam(
        config.get("ckanext.datagovau.spatialingestor.geoserver.os_user")
    ).pw_uid
    gid = grp.getgrnam(
        config.get("ckanext.datagovau.spatialingestor.geoserver.os_group")
    ).gr_gid
...
def _load_esri_shapefiles(shp_res, table_name, tempdir):
    ...
    logger.debug("Using SHP file " + shp_res["url"])

    if not any(
        [shp_res["url"].lower().endswith(x) for x in ["shp", "shapefile"]]
    ):
        ...
        urllib.urlretrieve(shp_res["url"], "input.shp")
        logger.debug("SHP downloaded")
...
def _load_kml_resources(kml_res, table_name):
    ...
    logger.debug("Using KML file " + kml_res["url"])
    native_crs = "EPSG:4326"
...
def _apply_sld(name, workspace, layer_name, url=None, filepath=None):
    ...
    if url:
        r = _make_request(
            requests.get,
            url,
            headers={"Authorization": "c3557c21-bfda-44e7-a5c3-c97de603d56b"},
        )
        if r and r.ok:
    ...
    if r and r.ok:
        ...
        r = _make_request(requests.delete, url, auth=(geo_user, geo_pass))
    ...
    r = _make_request(
        requests.post,
        url,
        data=json.dumps({"style": {"name": name, "filename": name + ".sld"}}),
    ...
    if r.status_code == 400:
        logger.info("Delete out old style in workspace")
        r = _make_request(
            requests.delete, style_url, auth=(geo_user, geo_pass)
        )
        r = _make_request(
            requests.put,
            url + "/" + name,
...
def _convert_resources(
    ...
    if len(shp_resources):
        native_crs = _load_esri_shapefiles(
            shp_resources[0], table_name, temp_dir
        )
        ...
        native_crs = _load_grid_resources(
            grid_resources[0], table_name, temp_dir
        )
...
def _perform_workspace_requests(datastore, workspace, table_name=None):
    ...
    if not r or not r.ok:
        _failure(
            "Failed to create Geoserver store {}: {}".format(
                _base_url, r.content
            )
        )
...
def _delete_resources(dataset):
    geoserver_resources = filter(
        lambda x: any(
            [
                old_urls in x["url"] and "/geoserver" in x["url"]
                for old_urls in ["dga.links.com.au", "data.gov.au"]
            ]
        ),
        dataset["resources"],
    )
...
def _prepare_everything(
    ...
    r = _make_request(requests.head, _ws_url, auth=(geo_user, geo_pass))
    ...
    if not r or not r.ok:
        _failure(
            "Failed to create Geoserver workspace {}: {}".format(
                _base_url, r.content
            )
        )
...
def check_if_may_skip(dataset_id, force=False):
    ...
    if (
        not dataset.get("organization", None)
        or "name" not in dataset["organization"]
    ):
        raise IngestionSkip(
    ...
    if (
        dataset.get("harvest_source_id", "") != ""
        or str(dataset.get("spatial_harvester", False)).lower()[0] == "t"
    ):
        raise IngestionSkip(
            "Harvested datasets are not eligible for ingestion"
        )

    if (
        str(dataset.get("private", False)).lower()[0] == "t"
        and dataset["name"] != "test333"
    ):
    ...
    activity_list = get_action("package_activity_list")(
        {"user": _get_username(), "model": model}, {"id": dataset["id"]}
    )
    ...
    if activity_list and activity_list[0]["user_id"] == user["id"]:
        if force:
            logger.info(
                "ignoring that we did last update because forced manual update"
            )
    ...
    if dataset:
        ...
        if (
            ...
            and (
                any(
                    [
                        "grid" in x["format"].lower()
                        for x in dataset["resources"]
                    ]
                )
                or any(
                    [
                        "geotif" in x["format"].lower()
                        for x in dataset["resources"]
                    ]
                )
            )
    ...
    try:
        ...
        bbox_obj = (
            _update_package_with_bbox(
                bbox, latlngbbox, layer_data, dataset, native_crs, bgjson
            )
            if bbox and not using_grid
            else None
        )
        ...
        if not r and not r.ok:
            _failure(
                "Failed to create Geoserver layer {}: {}".format(
                    _layer_base_url, r.content
                )
            )
        ...
        logger.error(
            "failed to ingest {0} with error {1}".format(dataset_id, e)
        )
