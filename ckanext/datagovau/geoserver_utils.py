...
class Geoserverobj():
    ...
    def get_geo_res_list(self, dataset):
        ...
        for format_group in self.res_group_formats:
            for resource in resources:
                ...
                is_source = True if fmt in self.res_group_formats[
                    format_group] else False
                ...
                if is_source:
                    if utils.contains(
                        ...
                        self.filtered_resources[format_group].append(
                            resource)
                        is_there_res = True
        return is_there_res            
                    ...
                    for res in rest_list:
                        if already_got_one:
                            ...
                            if res_import.ok:
                                ...
                                if import_data['import'].get('tasks'):
                                    ...
                                    if task['state'] != 'ERROR':
                                        ...
                                        if upd_l.ok:
                                            ...
                                            log.error('Status code is {0}'.format(upd_l.status_code))
                                ...
                                log.error(
                                    'Something went wrong while sending resource to import. Status code is {0}'.format(
    ...
    def upload_file_to_datastore(self, workspace, datastore, storage_type, file, filename, content_type, extension):
    ...
    def _clear_old_table(self, dataset, table_name) -> str:
        self.table_name = table_name
        log.info('Delete existing rows in DB for dataset {0}'.format(dataset['id']))
...
def _get_cursor():
    ...
    try:
        conn = psycopg2.connect(tk.config["ckanext.datagovau.spatialingestor.datastore.url"])
    except:
...
def delete_ingested(pkg_id):
    dataset_dict = tk.get_action(
        'package_show')({"user": tk.config.get(
                "ckanext.datagovau.spatialingestor.username", ''),
         "ignore_auth": False}, {'id': pkg_id})
