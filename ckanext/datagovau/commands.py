...
class SpatialIngestor(CkanCommand):
    ...
    def _ingest(self, scope):
        if scope in ("all", "updated", "updated-orgs"):
            ...
            for counter, pkg_id in enumerate(pkg_ids):
                sys.stdout.write(
                    "\rIngesting Package ID {0}/{1} ({2})\r".format(
    ...
    def _purge(self, scope):
        if scope in ["all", "erroneous"]:
            pkg_ids = [
                r[0] for r in model.Session.query(model.Package.id).all()
            ]
...
class ReconcileGeoserverAndDatastore(CkanCommand):
    ...
    def command(self):
        ...
        datastore_postgis_same = config.get(
            "ckanext.datagovau.datastore.url"
        ) == config.get("ckanext.datagovau.spatialingestor.postgis.url")

        dry_run = len(self.args) == 1 and self.args[0].lower() == "dry-run"
        clean_all = len(self.args) == 1 and self.args[0].lower() == "clean-all"
        clean_dbs = clean_all or (
            len(self.args) == 1 and self.args[0].lower() == "clean-dbs-only"
        )
        clean_fs = clean_all or (
            len(self.args) == 1 and self.args[0].lower() == "clean-fs-only"
        )
        clean_geoserver = clean_all or (
            len(self.args) == 1
            and self.args[0].lower() == "clean-geoserver-only"
        )
        clean_ckan_resources = clean_all or (
            len(self.args) == 1 and self.args[0].lower() == "clean-ckan-only"
        )
        ...
        for counter, ws in enumerate(geoserver_workspaces):
            ...
            try:
                ...
                coveragestores = [
                    r["name"]
                    for r in res.json()["coverageStores"]["coverageStore"]
                ]
                have_directory = True
                for cs in coveragestores:
                    ...
                    for cvg in [
                        r["name"] for r in res.json()["coverages"]["coverage"]
                    ]:
        ...
        sys.stdout.write(
            "\nGeoserver Vector Workspaces Without PostGIS Tables: {0}".format(
                ws_without_table_counter
            )
        )
        sys.stdout.write(
            "\nGeoserver Feature Types Without PostGIS Tables: {0}".format(
                ft_without_table_counter
            )
        )
        ...
        sys.stdout.write(
            "\nGeoserver Raster Workspaces Without Filestore Directories: {0}"
            .format(ws_without_directory_counter)
        )
        sys.stdout.write(
            "\nGeoserver Coverages Without Filestore Directories: {0}".format(
                cvg_without_directory_counter
            )
        )
        ...
        for counter, pkg_id in enumerate(pkg_ids):
            ...
            for res_dict in pkg_dict.get("resources", []):
                ...
                if "data.gov.au/geoserver" in res_dict.get("url", ""):
                    re_res = re.search(
                        ".*data\.gov\.au\/geoserver\/(.*)\/.*", res_dict["url"]
                    )
                elif "links.com.au/geoserver" in res_dict.get("url", ""):
                    re_res = re.search(
                        ".*links\.com\.au\/geoserver\/(.*)\/.*",
                        res_dict["url"],
                    )
                ...
                if re_res:
                    ...
                    if all(
                        [
                            ws_name in x
                            for x in [ws_without_table, ws_without_directory]
                        ]
                    ) or not (ws_name in geoserver_workspaces):
                ...
                if not res_delete:
                    ...
                    if new_label != "":
                        ...
                        try:
                            ...
                            if clean_ckan_resources:
                                toolkit.get_action("resource_update")(
                                    {"ignore_auth": True}, res_dict
                                )
                            active_datastore_tablenames.add(res_dict["id"])
                        except:
                            if new_label_bool:
                                sys.stdout.write(
                                    "\nResource {0} Failed Validation; Will"
                                    " Drop Associated Datastore Table".format(
                                        res_dict["id"]
                                    )
                                )
                            else:
                                sys.stdout.write(
                                    "\nResource {0} Failed Validation; Marking"
                                    " Resource For Deletion".format(
                                        res_dict["id"]
                                    )
                                )
                                resources_to_delete.add(res_dict["id"])
                                if ws_name:
                                    if ws_name in active_raster_workspaces:
                                        active_raster_workspaces.remove(
                                            ws_name
                                        )
                        ...
                        sys.stdout.write(
                            "\nProcessing Package ID {0}/{1}".format(
                                counter + 1, total
                            )
                        )
        ...
        sys.stdout.write(
            "\nActive Geoserver Resources: {0}.".format(
                len(active_raster_workspaces & active_vector_workspaces)
            )
        )
        sys.stdout.write(
            "\nActive Geoserver Raster Resources: {0}".format(
                active_raster_counter
            )
        )
        sys.stdout.write(
            "\nActive Geoserver Vector Resources: {0}".format(
                active_vector_counter
            )
        )
        sys.stdout.write(
            "\nActive Geoserver Resources With Broken Workspaces: {0}".format(
                delete_counter
            )
        )
        ...
        for counter, ws in enumerate(active_vector_workspaces):
            sys.stdout.write(
                "\rExtracting PostGIS Table Name From Geoserver Workspace"
                " {0}/{1}".format(counter + 1, total)
            )
            sys.stdout.flush()
            res = requests.get(
                geoserver_url + "rest/workspaces/" + ws + "/datastores",
        ...
        if datastore_postgis_same:
            sys.stdout.write(
                "\nFinding Active Geoserver Feature Types Without A PostGIS"
                " Table..."
            )
            geoserver_featuretypes_without_table = (
                active_geoserver_featuretypes - set(datastore_tables)
            )
            sys.stdout.write(
                " Found {0} Feature Types".format(
            ...
            valid_table_names = (
                set(active_datastore_tablenames)
                | active_geoserver_featuretypes
            )
            datastore_tables_to_drop = list(
                set(datastore_tables) - valid_table_names
            )
            sys.stdout.write(
                " Will Drop {0} Out Of {1} Tables".format(
            ...
            geoserver_featuretypes_without_table = (
                active_geoserver_featuretypes - set(postgis_tables)
            )
            sys.stdout.write(
                " Found {0} Feature Types".format(
                    len(geoserver_featuretypes_without_table)
        ...
        sys.stdout.write("\n----------")
        sys.stdout.write(
            "\nExtracting Filestore Directory Name From Geoserver"
            " Workspace 0/0"
        )
        for counter, ws in enumerate(active_raster_workspaces):
            sys.stdout.write(
                "\rExtracting Filestore Directory Name From Geoserver"
                " Workspace {0}/{1}".format(counter + 1, total)
            )
            sys.stdout.flush()
            res = requests.get(
        ...
        sys.stdout.write(
            "\nFinding Active Geoserver Coverages Without A Filestore"
            " Directory..."
        )
        geoserver_coverages_without_directory = (
            active_geoserver_coverages - set(filestore_directories)
        )
        sys.stdout.write(
            " Found {0} Coverages".format(
                len(geoserver_coverages_without_directory)
            )
        )
        ...
        filestore_directories_to_delete = list(
            set(filestore_directories) - valid_directory_names
        )
        sys.stdout.write(
            " Will Delete {0} Out Of {1} Directories".format(
                len(filestore_directories_to_delete),
                len(filestore_directories),
            )
        )
        ...
        sys.stdout.write(
            " Will Delete {0} Out Of {1} Workspaces".format(
        ...
        sys.stdout.write("\n----------")
        sys.stdout.write(
            "\nDirectories To Be Deleted From Filestore ({0} Out Of {1}):"
            .format(
                len(filestore_directories_to_delete),
                len(filestore_directories),
            )
